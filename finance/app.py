    ...
    if request.method == "POST":
        ...
        if users_cash[0]["cash"] < total_price:
            ...
            if not users_purchases:
                db.execute("UPDATE purchases SET id = ?, symbol = ?, price = ?, amount = ? VALUES ?, ?, ?, ?", users_id, symbol, share_price, number_of_shares)
            ...
            # If user is already exist in the purchase database, just update the purchase database
                ...
                if users_purchases[0]["symbol"] == symbol:
                    ...
                    db.execute("UPDATE purchases SET (price, amount) TO ?, ? WHERE id = ? AND symbol = ?", share_price, number_of_shares, users_id, symbol)
